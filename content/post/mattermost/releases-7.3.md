...
今までBoard機能の各ボードの権限はMattermostのチャンネルに基づいて管理されていましたが、今回のバージョンからRoleベースの権限管理に変更されました。

Board画面の右上に表示される **共有** ボタンから、Boardに対する権限設定を行うことができます。
...
Mattermostのチャンネル全体に権限を与えるには、ボードをMattermostチャンネルに**リンク**する必要があります。ボードとの**リンク**を行うと、そのMattermostチャンネルのメンバー全員にボードに対する**編集者(Editor)**権限を付与することができます。ボードは一つのMattermostチャンネルとリンクすることができ、Mattermostチャンネルとリンクした場合、チャンネル画面の右サイドバーに表示されるようになります。
...
Boardsのサイドバーが刷新され、Mattermost Channels(チャット機能)と同じような見た目になりました。  
...
以前のバージョン(画面右)では、プレイブックと実行(Run)がそれぞれ全画面で表示されてるため、それぞれの内容を確認するためには画面を切り替える必要がありました。今回のバージョン(画面左)からは、全てのプレイブックと実行(Run)をサイドバーから一覧することができるようにな、プレイブック、実行(Run)へのアクセスが容易になりました。
...
「**最もアクティブでないチャンネル**」では、活動のないチャンネルを確認することができます。活動の少ないチャンネルに対しては、再び投稿を作成して注意をひいたり、チャンネルを脱退するなどの判断に利用することができます。  
